...
def api_held_release_history(request):
    ...
    for source in releases:
        ...
        payload.append({
            ...
            "claim_count": int(mir.claim_count if mir is not None else (len(claims) if claims else (source.delivered_claims_count or 0))),
            "service_count": int(mir.service_count if mir is not None else (source.services_count or 0)),
