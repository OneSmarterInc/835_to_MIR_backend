...
def notice_workflow_status(notice):
    statuses = list(notice.notice_claims.values_list("workflow_status", flat=True))
    if not statuses or all(status == "YET_TO_START" for status in statuses):
        return "YET_TO_START"
