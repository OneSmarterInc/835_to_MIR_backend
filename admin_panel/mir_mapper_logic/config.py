...
PAID_CLAIM_REASON_CODES = {"CO22", "CO41", "PR78"}
ADJUSTMENT_FREQUENCY_CODES = {"6"}
STREAMLINE_FACILITY_TYPE = "AA"
...
if AUTH_MODE not in {"local", "required"}:
    raise RuntimeError("MIR_AUTH_MODE must be 'local' or 'required'.")
if any(host not in _LOOPBACK_HOSTS for host in ALLOWED_HOSTS):
    if AUTH_MODE != "required" or not AUTH_USERNAME or not AUTH_PASSWORD or not DOWNLOAD_SIGNING_KEY:
        raise RuntimeError(
            "A remotely accessible mapper requires MIR_AUTH_MODE=required, credentials, and MIR_DOWNLOAD_SIGNING_KEY."
        )
