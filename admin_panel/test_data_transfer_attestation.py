...
from admin_panel.data_transfer_attestation_service import (
    ...
    validate_signed_data_transfer_attestation,
)
...
def add_mock_signatures(pdf_bytes):
    ...
    for x in (105, 375):
        ...
        path.moveTo(x, 370)
        path.curveTo(x + 25, 400, x + 45, 350, x + 80, 386)
        path.curveTo(x + 100, 400, x + 120, 360, x + 145, 382)
...
class PersonalizedDataTransferAttestationTestCase(TestCase):
    def setUp(self):
    ...
    def test_step_two_download_contains_client_identity_date_and_signature_label(self):
        self.client.force_login(self.admin)
        response = self.client.get(
            f"/admin-panel/api/clients/{self.client_record.id}/golive/steps/2/download/"
        )
    ...
    def test_blank_personalized_attestation_rejects_both_signatures(self):
        ok, checks = validate_signed_data_transfer_attestation(
            build_client_data_transfer_attestation(self.client_record), self.client_record
        )
        self.assertFalse(ok)
    ...
    def test_attestation_with_both_signatures_is_accepted(self):
        signed_pdf = add_mock_signatures(build_client_data_transfer_attestation(self.client_record))
        ok, checks = validate_signed_data_transfer_attestation(signed_pdf, self.client_record)
