...
class PersonalizedGoLiveAuthorizationTestCase(TestCase):
    ...
    def test_repeated_authorization_download_builds_pdf_once(self):
        self.client.force_login(self.admin)
        url = f"/admin-panel/api/clients/{self.client_record.id}/golive/steps/1/download/"
        with patch(
            "admin_panel.golive_authorization_service.build_client_golive_authorization",
            wraps=build_client_golive_authorization,
        ) as builder:
