...
def cached_client_pdf(client, template_name, builder):
    ...
    # Validate before consulting either cache. Otherwise a PDF generated by an
    # older, less strict release could continue to bypass current requirements.
    from admin_panel.nda_service import client_nda_address

    if not str(getattr(client, "name", "") or "").strip():
        raise ValueError("Client legal name is required before downloading this document.")
    if not client_nda_address(client):
        raise ValueError("Client address is required before downloading this document.")
