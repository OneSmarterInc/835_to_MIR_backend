...
from accounts.admin_screens import (
    ADMIN_SCREENS, DEFAULT_ADMIN_SCREENS, SCREEN_LABELS,
    normalize_admin_screens, screens_for_user,
)
from edi835.models import EDI835File
from .models import OnboardingStepDefinition, ClientStepStatus, GoLiveStepDefinition, ClientGoLiveStatus, ClientTestEnvironment, AuditLog, AdminClientAccessGrant, ClientSmtpConfig, ClientDocument, log_audit_event
from project835.field_crypto import (
    encrypt_smtp_password,
    decrypt_smtp_password,
)
...
def api_admin_access_info(request):
    ...
    for u in visible_users:
        staff_list.append({
            ...
            "access": "Full Access" if u.is_superuser else ("Custom Admin Access" if u.is_staff else "Standard Access"),
    ...
    return JsonResponse({
        ...
        "staff": staff_list,
        "users": staff_list,
        "available_admin_screens": [
            {"key": key, "label": SCREEN_LABELS[key]} for key in ADMIN_SCREENS
        ],
        "active_client_grants": [{
            "id": grant.id, "user_id": grant.administrator_id,
            "client_id": str(grant.client_id), "client_name": grant.client.name,
            "reason": grant.reason, "expires_at": grant.expires_at.isoformat(),
...
def api_admin_revoke_client_access_grant(request, grant_id):
    ...
    if request.method != "POST":
        return JsonResponse({"success": False, "error": "Only POST is allowed."}, status=405)
    grant = AdminClientAccessGrant.objects.filter(pk=grant_id, revoked_at__isnull=True).first()
    if not grant:
        return JsonResponse({"success": False, "error": "Active grant not found."}, status=404)
...
def api_admin_create_user(request):
    ...
    if user.is_staff and not user.is_superuser:
        user.admin_screens = list(DEFAULT_ADMIN_SCREENS)
...
def api_admin_update_user(request, user_id):
    ...
    if "admin_screens" in data:
        if not request.user.is_superuser:
            return JsonResponse({"success": False, "error": "Only a Super Admin can change administrator screen access."}, status=403)
        if not isinstance(data["admin_screens"], list):
            return JsonResponse({"success": False, "error": "admin_screens must be a list."}, status=400)
        user_obj.admin_screens = normalize_admin_screens(data["admin_screens"])
    user_obj.is_staff = requested_is_staff
    user_obj.is_superuser = requested_is_superuser
    if user_obj.is_staff:
        user_obj.client = None
        if not user_obj.is_superuser and user_obj.admin_screens is None:
            user_obj.admin_screens = list(DEFAULT_ADMIN_SCREENS)
